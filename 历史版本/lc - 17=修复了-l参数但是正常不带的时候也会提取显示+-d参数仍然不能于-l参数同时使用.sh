...
eval $command | while read -r line; do
    ...
    if $small_output; then
        if [[ $recv_packets -ge $min_packets && $recv_bytes -ge $min_bytes ]]; then
        ...
        if [[ $recv_packets -ge $min_packets && $recv_bytes -ge $min_bytes ]]; then
            echo "$protocol $src_ip:$src_port -> $dst_ip:$dst_port recv_packets: $recv_packets recv_bytes: $recv_bytes send_packets: $send_packets send_bytes: $send_bytes"
        fi
    else
        if [[ $recv_packets -ge $min_packets && $recv_bytes -ge $min_bytes ]]; then
            echo "$protocol src=$src_ip sport=$src_port dst=$dst_ip dport=$dst_port packets=$recv_packets bytes=$recv_bytes"
        fi
    fi
done
