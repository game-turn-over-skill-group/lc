...
case $key in
    ...
    -b|-B|--bytes)
    if [[ $2 =~ ^[0-9]+$ ]]; then
        command="$command | awk '{for(i=1;i<=NF;i++){if($i ~ /^bytes=[0-9]+$/){split($i,a,"=");if(a[2]>='$2'){print}}}}'"
    fi
    shift
    shift
    ;;
    -s|-S|--packets)
    if [[ $2 =~ ^[0-9]+$ ]]; then
        command="$command | awk '{for(i=1;i<=NF;i++){if($i ~ /^packets=[0-9]+$/){split($i,a,"=");if(a[2]>='$2'){print}}}}'"
    fi
    shift
    shift
    ;;
