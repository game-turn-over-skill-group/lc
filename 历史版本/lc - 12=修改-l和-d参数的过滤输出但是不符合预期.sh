...
eval $command | while read -r line; do
    ...
    if [[ $packets =~ ^[0-9]+$ && $bytes =~ ^[0-9]+$ ]]; then
        if [ "$small_output" = true ]; then
            # 打印简化输出
            echo "$protocol $src_ip $src_port $dst_ip $dst_port $packets $bytes"
        elif [ "$debug_mode" = true ]; then
            # 打印调试信息
            echo "Debug: src_ip=$src_ip src_port=$src_port packets=$packets bytes=$bytes min_packets=$min_packets min_bytes=$min_bytes"
        else
            # 过滤条件
