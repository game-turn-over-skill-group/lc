...
if [ "$small_output" = true ]; then
    eval $command | awk -v min_packets="$min_packets" -v min_bytes="$min_bytes" '
    {
        ...
        for(i=1; i<=NF; i++) {
            if ($i ~ /^packets=/) {
                current_packets = substr($i, index($i, "=") + 1)
                if (current_packets > packets) {
                    packets = current_packets
                }
            }
            if ($i ~ /^bytes=/) {
                current_bytes = substr($i, index($i, "=") + 1)
        ...
        # 过滤条件
        if ((min_packets == 0 || packets >= min_packets) && (min_bytes == 0 || bytes >= min_bytes)) {
            print $0
        }
    }'
else
