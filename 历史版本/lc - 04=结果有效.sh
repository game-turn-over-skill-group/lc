...
case $key in
    ...
    -b|-B|--bytes)
    if [[ $2 =~ ^[0-9]+$ ]]; then
        command="$command | grep -E 'bytes=[0-9]+' | awk -F'bytes=' '{if(\$2 >= $2) print \$0}'"
    fi
    shift
    shift
    ;;
    -s|-S|--packets)
    if [[ $2 =~ ^[0-9]+$ ]]; then
        command="$command | grep -E 'packets=[0-9]+' | awk -F'packets=' '{if(\$2 >= $2) print \$0}'"
    fi
    shift
    shift
    ;;
...
if [ "$small_output" = true ]; then
    eval $command | awk '{print $1,$3,$6,$7,$8,$9,$10,$11,$16,$17}'
else
    eval $command
fi
