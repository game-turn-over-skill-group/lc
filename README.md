...
---
- [x] `-4`(数字4)：参数内容：为空(`null`)
* ###### 例子：lc -4
> 说明：查看ipv4连接表（等同于 -i ipv4）
---
- [x] `-6`(数字6)：参数内容：为空(`null`)
* ###### 例子：lc -6
> 说明：查看ipv6连接表（等同于 -i ipv6）
---
- [x] `-N`(大小写英文字母n ：`Network`)：参数内容：`tcp`/`udp`/`icmp`/`icmpv6`/`esp`/`……`
* ###### 例子：lc -n udp 或 lc -N udp	（大小写功能相同）
> 说明：查看udp类型的连接表
---
- [x] `-p`(小写英文字母p ：`ports`)：参数内容：`2710`,`6969` 或 `2710:6969`
* ###### 例子：lc -p 8888		（小写p用于显示端口）
> 说明：仅输出显示 自定义端口2710和6969端口的 连接表行条目
>
> 匹配显示 sport+dport 逗号分割支持多个端口
>
> 端口范围支持 `-` 或 `:` 分隔符，如 `6811-6924` 或 `6811:6924`
---
- [x] `-P`(大写英文字母P ：`Ports`)：参数内容：`666`,`888`,`6811-6924` 或 `6811:6924`
* ###### 例子：lc -P 6811-6924 或 lc -P 6811:6924	（大写P用于过滤端口）
> 说明：过滤不显示 这些端口/端口范围
>
> 匹配过滤 sport+dport 包含参数内容的端口号就过滤该行条目
>
> 端口范围支持 `-` 或 `:` 分隔符
---
